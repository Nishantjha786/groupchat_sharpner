...
trackerFrom.addEventListener('submit', async(e) => {
    ...
    try {   
        await axios.post(`http://localhost:5000/expense/add-expense`, {
            amount: amount.value,
...
async function fetchExpensesFromBackend(pageNo) {
    try {
        ...
        const response = await axios.get(`http://localhost:5000/expense/get-expense/?page=${pageNo}`, {
            headers: {
                'Authorization': localStorage.getItem('token'),
                'rows': rows
            }
...
document.getElementById('rzp-button1').onclick = async function (e) {
    ...
    try {
        ...
        let response = await axios.post(`http://localhost:5000/user/purchase-premium`, {}, {
            headers: {
                'Authorization': token
            }
        });
        ...
        var options = {
            ...
            "handler": function (response) {
                console.log(response);
                axios.post(`http://localhost:5000/user/purchase-premium/update-transaction-status`,
                    {
...
async function checkForPremium() {
    try {
        ...
        const response = await axios.post(`http://localhost:5000/user/check-membership`, {}, {
            headers: {
                'Authorization': token
            }
        });
...
async function addLeaderboard() {
    try {
        ...
        const response = await axios.get(`http://localhost:5000/expense/get-leaderboard`, {
            headers: {
                'Authorization': localStorage.getItem('token')
            }
        });
...
async function showPreviousDownloads() {
    try {
        ...
        const response = await axios.get(`http://localhost:5000/user/get-downloads`, {
            headers: {
                'Authorization': localStorage.getItem('token')
            }
        });
...
async function expandExpense(id) {
    try {
        let response = await axios.get(`http://localhost:5000/user/get-expansion/${id}`, {
            headers: {
                'Authorization': localStorage.getItem('token')
            }
        });
...
function download(){
    axios.get(`http://localhost:5000/user/download`, 
        { 
